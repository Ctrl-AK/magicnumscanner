...
class MagicNumScanner
  ...
  attr_accessor :virtual_file, :virtual_file_mem, :report
  ...
  def generate_report(filetype, signature)
    ...
    if filetype_url.include?("|")
      ...
      filetype_url.each do |filetype_ext|
        returned+="\nFound: #{filetype} | More info: http://filext.com/file-extension/#{filetype_ext}" # | Magic: #{signature}"
      end
    else
      returned+="\nFound: #{filetype} | More info: http://filext.com/file-extension/#{filetype_url}" # | Magic: #{signature}"
    end
    returned
  end  
  ...
  def sig_check
    ...
    SIGN.each do |filetype, signature|
      ...
      if @virtual_file_mem[0..signature.size-1] == signature
        @report+=generate_report(filetype, signature)
      end
    end
    $binfile.close
    @virtual_file.clear
  ...
  def action!(file)
     if file.nil?
       ...
       begin
         ...
         @report=sig_check # do signature check
       end
     end
  end
